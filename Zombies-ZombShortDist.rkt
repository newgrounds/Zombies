...
;; This version is most fun because the player can't go
;; off the edge of the board, but the Zombies can!!!!
...
(big-bang (new world% 
               (list) 
               (zombs (+ 5 (random 8))) 
               (new player% 0 0 0 0 PLAYER-COLOR)))


;; While realizing it is completely unacceptable to ignore test
;; cases for all of these functions, we can't fix the fact that
;; we have simply run out of time to include them

#|
;; TEST CASES

;; Zombie tests
(check-expect (send (new zombie% 100 100 0 0 false "red")
                    die? (list (new zombie% 102 102 0 0 true "gray")
                                (new zombie% 100 100 0 0 true "gray")))
              true)
(check-expect (send (new zombie% 100 100 0 0 false "red") draw)
              (circle 10 "solid" "red"))
(check-expect (send (new zombie% 100 100 100 100 false "red") move)
              (new zombie% 100 100 100 100 false "red"))

;; Player tests
(check-expect (send (new modulo-player% 100 100 100 100) move)
              (new modulo-player% 100 100 100 100))
(check-expect (send (new modulo-player% 100 100 0 0) draw)
              (place-image (circle 10 "solid" "green")
                           100 100
                           (empty-scene 400 400)))

;; A sample world to use for the World test cases
(define world1 (new world%
                    (list)
                    (list (new zombie% 100 100 100 100 "red"))
                    (new modulo-player% 100 100 100 100)))
(check-expect (send world1 on-mouse 101 101 "move")
              (new world% 
                   (list (new zombie% 100 100 100 100 "red"))
                   (new modulo-player% 100 100 101 101)))
(check-expect (send world1 to-draw)
              (place-image (circle 10 "solid" "red")
                           100 100
                           (place-image (circle 10 "solid" "green")
                                        100 100
                                        (empty-scene 400 400))))
(check-expect (send world1 name) "Zombie!")
(check-expect (send world1 on-tick)
              (new world%
                   ...
                   (list (new zombie% 
                              ...
                              "red"))
                   (new modulo-player% 100 100 100 100)))
(check-expect (send world1 stop-when)
              true)
|#